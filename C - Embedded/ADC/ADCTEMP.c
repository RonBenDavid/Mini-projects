...
{
  // DAC12 configuration
  DAC12_0DAT =0;                         // DAC_0 output 0V

  ADC12CTL0 =REF2_5V;                  // Internal 2.5V ref on
  
  DAC12_0CTL =0x01A2 ; // DAC_0 -> P6.6
  				                                // DAC_1 -> P6.7
                                                // DAC reference Vref
                                                // 12 bits resolution
                                                // Immediate load
...
void ADC12Init(void)
{
 ...
 //ADC12CTL1 -> Control Register 1
  ...
  //ADC12BUSY (ADC12 busy)                   ->  read only  
    ADC12CTL1 =0 ;   // Start MEM0, TB1, Rpt Sing.
